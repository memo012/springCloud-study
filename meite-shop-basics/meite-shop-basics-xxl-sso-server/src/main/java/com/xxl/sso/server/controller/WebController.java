...
//        }
        ...
        BaseResponse<UserOutDTO> ssLogin = memberServiceFeign.ssoLogin(userLoginInpDTO);
        if (!isSuccess(ssLogin)) {
            redirectAttributes.addAttribute("errorMsg", ssLogin.getMsg());
            redirectAttributes.addAttribute(Conf.REDIRECT_URL, request.getParameter(Conf.REDIRECT_URL));
            return "redirect:/login";
        }
        UserOutDTO data = ssLogin.getData();
        if (data == null) {
        ...
        xxlUser.setUserid(data.getUserId() + "");
